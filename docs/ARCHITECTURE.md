...
- **Merge order** is the app's defaults, then the extensions, then the configuration document. An extension cannot displace an app's message, flag, permission or endpoint alias, and a deployment can still override an extension's flag default, roles or copy, provided the app's configuration schema accepts the key (the reference app's `flags` only accepts its own flag names).
