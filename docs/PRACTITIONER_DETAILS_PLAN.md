...
Scope: read screens only (`UserDetailsDrawer`, `UserEditDrawer`). Writes stay as Bundle transactions plus an audit write.
Backend: `ohs-player-reference-backend` `main` (endpoint landed in #70, commit `3c8b617`).
Base branch: **`v1.0.0-alpha`**, which is where the portal's active work lands. `main` is 19 commits behind and is not the development line.
...
**F4 — `UserDetailsDrawer` has no error state today.** `read.error` is ignored. The ticket requires one; reuse `features/sdc/toErrorMessage.ts` (already does the `instanceof FhirError` → `formatOperationOutcomeMessage` narrowing) with the app `ErrorState` primitive. Do not re-implement the narrowing. On this branch `FhirClient.toError` already lifts the gateway's `{ error }` body into `error.message`, and `toErrorMessage` already falls back to it, so no error-plumbing change is needed here.
...
House conventions on this branch, followed by the port: icons come from `components/ui/icons`, transaction commits go through `commitBundle(client, entries)`, audit writes go through the app's `useWriteAudit()` hook, and the spacing token is `--ohs-sys-spacing-8`, which exists here in `theme.css`.
...
- `toErrorMessage.ts` — the gateway `{ error }` body is already handled on this branch.
...
7. `main` and `v1.0.0-alpha` have diverged: alpha is 19 commits ahead, `main` carries one commit alpha lacks (the medplum fhirtypes migration, partly redone on alpha). The default branch should be reconciled so PRs stop being aimed at the stale line.
