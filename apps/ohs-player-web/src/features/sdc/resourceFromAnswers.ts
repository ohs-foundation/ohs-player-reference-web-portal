...
export interface OrgFormFields {
  ...
  typeCode: string;
  /** Identifier value, or '' to omit (e.g. when autogenerated server-side). */
 ...
 * On edit, pass the existing resource so unmanaged fields (id, meta, partOf, …) survive while
 * form-managed fields are overwritten — including removals (cleared type/identifier/email).
 */
export function organizationFromForm(
  ...
  const idVal = fields.identifierValue.trim();
  const otherIds = Array.isArray(existing?.identifier)
    ? (existing.identifier as Identifier[]).filter((i) => i.system !== ORG_IDENTIFIER_SYSTEM)
    : [];
  const identifier = idVal ? [...otherIds, { system: ORG_IDENTIFIER_SYSTEM, value: idVal }] : otherIds;
  if (identifier.length > 0) org.identifier = identifier;
  else delete org.identifier;
